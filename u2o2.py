...
Notes:
   ...
   * DFH: swapped the order of attributes in chapter and verse elements
          sID now precedes osisID such that it aligns well with eID in the XML
          code that formerly tested for osisID has been edited to test for sID

   * DFH: removed the space before "/>" throughout the script
   
   * DFH: Changed XSD for validation to the updated CrossWire version instead of
...
Neither jython nor pypy are recommended as they are quite a bit slower at
...
TITLETAGS = {
    ...
    #
    # DFH: There is no value in using the reference element for section ranges
    # Having an OSIS link to click in a SWORD front-end is probably redundant
		# I may revert these, but if I do, I will include type="x-scope" in the reference
    # r'\mr': ('<title type="scope"><reference type="x-scope">', '</reference></title>'),
    # r'\sr': ('<title type="scope"><reference type="x-scope">', '</reference></title>'),
    r'\mr': ('<title type="scope">', '</title>'),
    r'\sr': ('<title type="scope">', '</title>'),
    # Having an OSIS link for parallel passages still makes good sense
    r'\r': ('<title type="parallel"><reference type="parallel">',
            '</reference></title>'),
    # USFM \sp tag represents a printed non-canonical secondary title,
    # whereas the OSIS <speaker> tag is indended to hold a canonical name
    # associated with <speech> elements. Disabled Aydeth's replacement.
    # r'\sp': ('<speaker>', '</speaker>'),
    # Using the replacement from usfm2osis.py instead
    r'\sp': ('<title level="2" subType="x-speaker">', '</title>'),
    #
    # ##### Descriptive Title = Canonical Psalm Title #####
    # DFH: As far as I can tell... (correct me if I'm wrong)
    #  This only covers when they are standalone title
    #  This script doesn't yet handle the situation when the title
    #  is all or part of the text for verse 1 (or more) as is the case
    #  for Bibles with the Greek numbering & versification for Psalms
    #  Even if it did, there's a further problem in that SWORD doesn't
    #  really allow a title to contain a verse (or part of a verse)
    #  even though a verse can contain a (mid-verse) title
    ...
    #       software and create problems with displaying other titles as well.
    ...
    # ---------------------------------------------------------------
    # DFH:  I recommend the following in place of what Aydeth decided above.
    #       This works with OSIS and with SWORD.
    r'\cl': ('<title type="chapter" subType="x-chapterLabel">', '</title>'),

    # ##### chapter cd tags ##### #
    # the osis user manual says cd titles should be in an introduction div.
    # DFH: chapter descriptions often contain several verse numbers or ranges
    #  it would be nice enhancement if these could be made into references
    #  even though they are not usually marked as such in SFM
...
PARTAGS = {
    ...
    # \pr and \ph# are deprecated in USFM 2.4
...
SPECIALTEXT = {
    ...
    # DFH: \addpn is for Chinese words to be dot underline
    #      not documented in USFM 2.4 but found in some usfm.sty
    r'\add': ('<transChange type="added">', '</transChange>'),
    r'\addpn': ('<transChange type="added" subType="x-usfm-addpn">', '</transChange>'),
    ...
    #       aiming to avoid invalid OSIS such as might occur because (e.g.)
    #       the divineName element is not allowed within the name element.
    #       this may be overkill but it seems to be fairly safe
    r'\+add': ('<seg type="x-nested"><transChange type="added">', '</transChange></seg>'),
    r'\+addpn': ('<seg type="x-nested"><transChange type="added" subType="x-usfm-addpn">', '</transChange></seg>'),
    r'\+nd': ('<seg type="x-nested"><divineName>', '</divineName></seg>'),
    r'\+pn': ('<seg type="x-nested"><name>', '</name></seg>'),
    r'\+qt': ('<seg type="otPassage" subType="x-nested">', '</seg>'),
    ...
    r'\+k': ('<seg type="keyword" subType="x-nested">', '</seg>'),
    ...
    # DFH: these do not validate if they occur within titles
    #      a solution would be to wrap them within a seg element
    #      better to use nested tags in the USFM for such cases (see below)
    ...
    # as above but for nested tags
    # DFH : wrapped all of these within a seg element with type="x-nested"
    ...
    # a few stray introduction and poetry tags
    # that work well being handled in this section.
...
NOTETAGS = {
    ...
    # DFH: added subType="x-extended" for traceability
...
NOTETAGS2 = {
    ...
    # DFH: Does \fm need further special handling ?
    #      An additional footnote marker location for a previous footnote
    #      How would this work in OSIS and SWORD ?
    r'\fdc': ('<seg editions="dc">', '</seg>'),
    r'\fr': ('<reference type="annotateRef">', '</reference>'),
    # Catchwords and catchphrases are those parts of notes 
    #   that are copied from the main text,
    #    to orient the reader as to the note's precise applicability.
    r'\fk': ('<catchWord>', '</catchWord>'),
    # DFH: How to add the optional osisRef value for the catchWord?
    r'\fq': ('<catchWord>', '</catchWord>'),
    r'\fqa': ('<rdg type="alternate">', '</rdg>'),
    # Aydeth: I think this should be label... but that doesn't validate.
    # DFH:    I wrapped it within a seg element - this might just validate.
    r'\fl': ('<seg type="x-footnoteLabel"><label>', '</label></seg>'),
    ...
    # Aydeth: currently preferred handling of xo and xt...
    # r'\xo': ('', ''),
    # DFH: provisionally marking xo using a seg element because 
    # we do need something to separate the origin from any non-reference text
    # that might precede the real scripture references in poorly constructed USFM
...
def reflow(text):
    ...
    if mangletext:
        ...
        # ##### this certainly needs more work #####
        # DFH: If a list spans a chapter division, the first part of the list
        #  should be terminated before the chapter element.
        #  Merely inserting a newline is insufficient!
        #  A paragraph tag would suffice for this purpose,
        #  but the translator may not have included one.
        for i in range(len(textlines)):
...
def convert_to_osis(text, bookid='TEST'):
    ...
    def preprocess(text):
        ...
        if r'//' in text:
            ...
            # DFH: Based on pages 73 & 119 of the OSIS User Manual
            text = text.replace(r'//', '<milestone type="screen"/>')
        if r'\pb' in text:
    ...
    def titlepar(text):
        ...
        # process paragraphs
            ...
            # finish paragraphs.
            text = u'{}{}{}\ufdd0'.format(text, pend, btag)

        # process tables
        # ##### this certainly needs more work #####
        # DFH: If a table extends to a chapter tag, the table
        #  should be terminated before the chapter element.
        #  A paragraph tag would suffice for this purpose,
        #  but the translator may not have included one.
        elif line[0] == r'\tr':
    ...
    def fixgroupings(lines):
        ...
        for i in range(len(lines)):
            if lines[i].startswith('<l '):
                ...
                if inlg:
                    lines[i - 1] = u'{}\ufdd0</lg>\ufdd0'.format(lines[i - 1])
                    inlg = False
        ...
        for i in range(len(lines)):
            if lines[i].startswith('<row'):
                ...
                if intable:
                    lines[i - 1] = u'{}\ufdd0</table>\ufdd0'.format(lines[i - 1])
                    intable = False
        ...
        for i in range(len(lines)):
            if lines[i].startswith(u'\ufdd0<!-- is2 '):
                ...
                lines[i] = '{}{}{}'.format(closeis2, closeis1, lines[i])
                closeis1 = ''
                closeis2 = ''
            elif u'\ufde1' in lines[i]:
                lines[i] = '{}{}{}'.format(closeis2, closeis1, lines[i])
    ...
    def chapverse(lines):
        ...
        for i in cvlist:
            ...
            if lines[i].startswith(r'\c '):
                ...
                # DFH: swapped attributes in chapter element
                if chap == '':
            ...
            # NB. but the example in the USFM Reference also has a line starts with \d
                ...
                # generate verse tag
                # DFH: swapped attributes in verse element
                if verse == '':
    ...
    def postprocess(lines):
        '''
        fix some ordering issues that may be present after processing
        '''
        ...
        # DFH: edited to suit swapped attributes in verse element
        for i in [_ for _ in range(len(lines)) if
        ...
        # DFH: edited to match swapped attributes in chapter element
        # case 1 = paragraphs only (improved to include p with an attribute)
        for i in [_ for _ in range(len(lines)) if
                  lines[_].startswith('<chapter sID')]:
            try:
                if lines[i + 1] == '</p>' and \
                   lines[i + 2].startswith('<p'):
                    lines.insert(i + 2, lines.pop(i))
            except IndexError:
                pass
        
        # case 2 = paragraph and divs
        for i in [_ for _ in range(len(lines)) if
                  lines[_].startswith('<chapter sID')]:
            try:
                if lines[i + 1] == '</p>' and \
        ...
        # fix issue where some chapter start tags had div's appended...
        # DFH: edited to match swapped attributes in chapter element
        for i in [_ for _ in range(len(lines)) if
                  lines[_].startswith('<chapter sID')]:
            try:
        ...
        # DFH: process parentheses in parallel passage and scope titles
        # move "(" to before <reference & move ")" to after </reference>
        # TBD.
...
def processfiles(args):
    ...
    # DFH: Disable normalization is now command line option -n
    #      "Unicode normalization can break Biblical Hebrew"
    #      (from the SBL Hebrew Font User Manual)
    if not args.n:
    ...
    # write doc to file
    outfile = '{}.osis.xml'.format(args.workid)
...
def main():
    ...
    parser.add_argument('-n',
                        help='disable Unicode normalization (default is to NFC)',
                        action='store_true')
